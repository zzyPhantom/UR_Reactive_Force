    ...
    def vision_position(self):
        ...
        if time.time() >=2:
            ...
            if self.ids is not None:
                ...
                # 计算旋转矩阵
                R_matrix, _ = cv2.Rodrigues(self.rvec)
                # 创建一个 Rotation 对象
                ...
                transform.transform.translation.x = float(self.tvec[0][0])
                transform.transform.translation.y = float(self.tvec[0][1])
                transform.transform.translation.z = float(self.tvec[0][2])
                ...
                transform.header.frame_id = 'camera_frame'
                transform.child_frame_id = f'aruco_frame'
                transform.transform.translation.x = float(0.0)
                transform.transform.translation.y = float(0.0)
                transform.transform.translation.z = float(0.0)
