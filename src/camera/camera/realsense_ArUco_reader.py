...
class Aruco_reader(Node):
    def __init__(self):
        ...
        # 创建相机的 tf2 Buffer 和 TransformListener
    ...
    def camera_pose_callback(self):
        try:
            # 从 camera_frame 到 aruco_frame 查询变换
            now = rclpy.time.Time()
            transform = self.tf_buffer.lookup_transform('base', 'camera_frame', now)
            ...
            # 创建齐次变换矩阵
            self.camera_pose[0:3, 0:3] = rotation_matrix  # 设置旋转矩阵部分
            self.camera_pose[0:3, 3] = [translation.x, translation.y, translation.z]  # 设置平移部分

        except TransformException as ex:
            self.get_logger().error(f"Could not read camera transform: {ex}")
    ...
    def vision_position(self):
        ...
        if time.time() >=2:
            ...
            if self.ids is not None:
                ...
                T_camera_to_aruco[:3, 3] = [self.tvec[0][0], self.tvec[0][1], self.tvec[0][2]]  # 设置平移向量

                # 机器人基坐标系到相机坐标系的齐次变换矩阵
                T_base_to_camera = self.camera_pose
                ...
                # self.trolley_position = [0.5, 0.0, 1.0, 3.14, 0.0, 0.0]
                # Broadcast TF
                transform = TransformStamped()
                transform.header.stamp = self.get_clock().now().to_msg()
